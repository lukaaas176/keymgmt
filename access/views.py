    ...
    for f in files:
        ...
        if r["format"] == "matrix":
            ...
            if r["corrected"]:
                ...
                parts.append(f"{len(r['corrected'])} scan-read serial(s) "
            ...
            if r["doors_matched_fuzzy"]:
                parts.append(f"{len(r['doors_matched_fuzzy'])} door name(s) "
                             f"matched to existing locks despite OCR noise")
            messages.success(
                request, f"{f.name}: locking matrix — {'; '.join(parts)}.")
            for w in r["warnings"]:
                messages.warning(request, f"{f.name}: {w}")
        else:
            added, updated = added + r["created"], updated + (not r["created"])
            note = "" if r["consistent"] else (
                f" — but the printout states {r['stated']} rows, "
                f"so please re-check")
            messages.success(
                request,
                f"{f.name}: {r['label']} — {r['parsed']} doors{note}.")
...
def dashboard(request):
    transponders = (Transponder.objects.annotate(n=Count("locks"))
...
def transponder_detail(request, serial):
    ...
    grouped = sorted((loc, sorted(v, key=lambda x: (x["lock"].door_name,
                                                    x["lock"].serial)))
    ...
    for loc, locks in _door_sections():
        rows = [{"lock": lk, "on": lk.serial in desired,
                 "inherited": lk.serial in inherited_from,
                 "via": ", ".join(inherited_from.get(lk.serial, [])),
                 "state": "active" if lk.serial in active
                 else ("planned" if lk.serial in planned else "")}
                for lk in locks]
        sections.append({"location": loc, "rows": rows})
    all_groups = [{"id": g.id, "name": g.name, "on": g.id in tp_group_ids}
                  for g in Group.objects.all()]
    return render(request, "access/transponder_detail.html", {
        "tp": tp, "grouped": grouped, "sections": sections,
        "all_groups": all_groups, "desired_total": len(desired),
        "active_total": len(active), "planned_total": len(planned),
        "removed_total": len(removed),
        "lock_total": len(active | planned), "nav": "transponders",
    })
...
def lock_list(request):
    locks = (Lock.objects.annotate(n=Count("transponders"))
             .order_by("location", "door_name", "serial"))
    return render(request, "access/lock_list.html",
                  {"locks": locks, "nav": "locks"})
...
def lock_detail(request, serial):
    ...
    all_groups = [{"id": g.id, "name": g.name, "on": g.id in lock_group_ids}
                  for g in Group.objects.all()]
    ...
    desirers = list(lock.desired_transponders.prefetch_related("groups")
                    .order_by(*order))
    ...
    status_cols = [
        {"title": "Bleibt", "hint": "aktiv und im Soll", "items": keep,
         "dot": "bg-emerald-500", "ring": "ring-emerald-200 bg-emerald-50/40",
         "count_cls": "text-emerald-600"},
        {"title": "Wird entfernt", "hint": "hohles × (entzogen) oder nicht im Soll",
         "items": remove, "dot": "bg-rose-500",
         "ring": "ring-rose-200 bg-rose-50/60", "count_cls": "text-rose-600"},
        {"title": "Geplant", "hint": "wird am Terminal programmiert",
         "items": [item(t) for t in planned], "dot": "bg-amber-400",
         "ring": "ring-amber-200 bg-amber-50/60", "count_cls": "text-amber-600"},
    ]

    return render(request, "access/lock_detail.html", {
        "lock": lock, "holder_total": len(active),
        "all_groups": all_groups, "desirers": individual_desirers,
        "addable": Transponder.objects.exclude(serial__in=desired_serials)
        .order_by(*order),
...
def overlap(request):
    ...
    * ``diff``    — only the pending grants (planned), i.e. who shares the
    ...
    if scope not in ("active", "planned", "diff"):
        scope = "active"
    tps = list(Transponder.objects
               .prefetch_related("locks", "planned_locks")
               .order_by("asta_number", "person_name", "serial"))
    ...
    clone_groups = [{"transponders": g, "doors": len(dset)}
                    for dset, g in by_set.items() if len(g) > 1]
    ...
    tp_data = [{"serial": tp.serial, "label": tp.label,
                "locks": sorted(sets[tp.serial])} for tp in tps]
    lock_names = {lk.serial: lk.label for lk in all_locks.values()}

    return render(request, "access/overlap.html", {
        "tps": tps, "rows": list(zip(tps, matrix)),
        "clone_groups": clone_groups,
        "tiers": tiers[:12], "tp_data": tp_data, "lock_names": lock_names,
        "accent_rgb": ACCENT_RGB, "nav": "overlap",
        "scope": scope, "planned_pending": planned_pending,
    })
...
def soll_matrix(request):
    ...
    for loc, locks in _door_sections():
        rows = []
        for lk in locks:
            cells = [{"kind": "group", "id": g.id,
                      "on": lk.serial in group_doors[g.id]} for g in groups]
            cells += [{"kind": "tp", "id": t.serial,
    ...
    tp_options = [{"serial": t.serial, "label": t.label}
                  for t in Transponder.objects.order_by(
                      "asta_number", "person_name", "serial")
                  if t.serial not in shown]
    return render(request, "access/soll_matrix.html", {
        "groups": groups, "tp_cols": tp_cols, "columns": columns,
        "sections": sections, "tp_options": tp_options,
        "tp_param": ",".join(t.serial for t in tp_cols), "nav": "soll",
    })
...
def group_create(request):
    d = _json_body(request)
    name = ((d.get("name") if isinstance(d, dict) else None) or "").strip()
    if not name:
...
def group_detail(request, pk):
    ...
    sections = []
    for loc, locks in _door_sections():
        sections.append({"location": loc, "rows": [
            {"lock": lk, "on": lk.serial in in_group} for lk in locks]})
    return render(request, "access/group_detail.html", {
        "group": group, "sections": sections,
        "members": group.transponders.order_by(
            "asta_number", "person_name", "serial"), "nav": "soll"})

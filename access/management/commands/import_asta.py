...
class Command(BaseCommand):
    help = (
        "Rebuild transponders/locks/grants from a CSV matrix export + its "
    ...
    def handle(self, *args, **opts):
        if not opts["wipe"]:
            raise CommandError(
                "this replaces the whole database; re-run with --wipe to confirm."
            )
        try:
            r = import_asta_csv(
                opts["csv"], opts["pdf"], source_name=os.path.basename(opts["csv"])
            )

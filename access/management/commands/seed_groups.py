...
GROUP_DOORS = {
    ...
    "AStA Lager": [
        "Mensa Raum -1004 ASTA Keller",
        "Mensa Vorhangschloss 1",
        "Mensa Vorhangschloss 2",
        "Notausgang Ost 001",
        "Notausgang Süd 012",
        "Eingang -111 ( Keller )",
        "Raum - 101 lager",
        ...
        "Raum - 108 ( Technik )",
        "Raum - 110 ( Technik )",
        "Flur Bau 3 z. Bau 8 West",
        "Flur Bau 3 z. Bau 8 Ost",
        "A -1307 E Flur Ost zur Architekturmuseum",
        "A -1307 E Flur West zur Architekturmuseum",
        "A Raum 0307 zur Keller",
        "Raum -1818 b Säureraum",
        "Raum -1818 zur Halle",
        "Flur zu Bau 3 Ost",
        "Flur zu Bau 3 West",
    ],
...
_add(
    "0XEHF6 0XEHKA 0XGTD5 0XHHSC 0XKCGX 0XRFKE 106LDU 106NRT 10A0CS 10A112 "
    "10A1N6 10A2A2 10A2AS 10BB5L 10BF7C 10BMLP 1USCHF 1USN9R 1USNTP 1USRUH "
    "1USSC2 1USTK6 1USUUT 1UTRPX 1UUTM5 1X0ALB 1XEUPT 1XNP2T 2UAA4K 2UAA55 "
    "2UBGL3 2UFSDU 2UFU56 2UL0C2 2UM6GX 2UP4BA 2URHK6 2XA73H 2XB29R 2XBC35 "
    "2XPKNR 3THFLX 3TR9UX 3TRE66 3TRUH3 3TS83N 3U02LU 3U3E6L 3UAHSN 3UAP0B "
    "3UAR11 3UB9FL 3UCB91",
    ["AStA Allgemein"],
)
_add("2UEHS1 2UH7LR 2UKX0E 2XEU1B 3UAG03 3UCC4E", ["AStA Allgemein", "AStA Lager"])
_add(
    "1USX8A 2UH4PG 2UL6P5 2XCT95 3TN2G5 3UCEAK", ["AStA Allgemein", "AStA Umweltlager"]
)
_add(
    "0XKK2X 11ELMN 2UAAUR 2UAUH6 2UK9KC 3T5CTL 3U35MC 3U50U4",
    ["AStA Allgemein", "AStA Technik"],
)
_add(
    "2UAC5U 2UEE9D 3U4345 3UB67H",
    ["AStA Allgemein", "AStA Lager", "AStA StudiTUM General"],
)
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **opts):
        ...
        by_name = {norm(lock.door_name): lock for lock in Lock.objects.all()}
        g43 = [
            lock
            for lock in Lock.objects.all()
            if "g43" in ((lock.location or "") + " " + (lock.area or "")).lower()
            or "gab 43" in ((lock.location or "") + " " + (lock.area or "")).lower()
        ]
        ...
        for name, door_names in GROUP_DOORS.items():
            make_implicit = (
                name == "AStA Allgemein"
                and not Group.objects.filter(is_implicit=True).exists()
            )
            g, _ = Group.objects.get_or_create(
                name=name, defaults={"is_implicit": make_implicit}
            )
            doors = (
                g43
                if name == "AStA StudiTUM General"
                else [by_name[norm(dn)] for dn in door_names if norm(dn) in by_name]
            )
            g.doors.set(doors)
            groups[name] = g
